...
import { Search, MapPin, Star, Clock, ChevronRight, SlidersHorizontal, X, Crosshair } from "lucide-react-native";
import { mockClubs, Club } from "../data/mockClubs";
import { BadgeChip } from "../components/shared/BadgeChip";
import { colors } from "../theme";

const USER_LAT = 43.2382;
const USER_LNG = 76.9454;
...
export function MapScreen() {
  ...
  }, [searchQuery, searchRadius, activeFilters]);
    ...
    <View style={{ flex: 1, backgroundColor: colors.bgDeep }}>
      ...
      >
        <Circle
          ...
          strokeWidth={1.5}
        />
        <Marker coordinate={{ latitude: USER_LAT, longitude: USER_LNG }} title="You are here">
        ...
        {filteredClubs.map((club) => (
          <Marker
            key={club.id}
            coordinate={{ latitude: club.lat ?? USER_LAT, longitude: club.lng ?? USER_LNG }}
      ...
      <View style={{ position: "absolute", top: 0, left: 0, right: 0, padding: 16, backgroundColor: "rgba(13,17,23,0.85)", gap: 12 }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <View style={{ flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 12, paddingVertical: 4, borderRadius: 999, backgroundColor: "rgba(26,26,46,0.9)", borderWidth: 1, borderColor: "rgba(124,58,237,0.3)" }}>
            <MapPin size={12} color={colors.cyan} />
            <Text style={{ fontSize: 12, color: colors.purpleLight, fontWeight: "600" }}>{searchRadius}km radius</Text>
          </View>
        </View>
